我先把 `maxResultSizeChars` 的真正落地链路补全：它在字段定义里只是入口，关键还要看结果预算、落盘替换、以及最终回给模型的包装流程。接下来我会并行把预算实现、调用链和展示层几处一起查出来，再整理成一份可直接保存成 Markdown 的技术文档。
[查找文件中]
[查找文件中]
[查找文件中]
[查找文件中]
我已经把主链路定位到 `toolResultStorage` 和 `query.ts` 了；现在再补两块关键证据：一是全局常量与聚合预算细节，二是哪些工具会选择 `Infinity` 或较小阈值，这样就能把“原理、边界、为什么这样设计”讲完整。
[读取文件中]
[读取文件中]
[查找文件中]
[思考中]
...
> 基于 [`Tool.ts](/Users/aedanxu/Downloads/claude-code-sourcemap-main/restored-src/src/Tool.ts)、[toolResultStorage.ts](/Users/aedanxu/Downloads/claude-code-sourcemap-main/restored-src/src/utils/toolResultStorage.ts)、toolLimits.ts` 的实现整理。  
> 可直接作为 Markdown 技术文档保存使用。
